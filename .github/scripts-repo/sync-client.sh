...
for CLIENT in "${CLIENTS[@]}"; do
  ...
  # Define paths to sync - now includes the applications directory
  SYNC_PATHS=(
    "charts"
    "values/${CLIENT}"
    "applications/${CLIENT}"
  ...
  if [ "${HTTP_CODE}" != "200" ]; then
    ...
    curl -s -X POST \
      ...
      -d "{
        ...
        \"private\": true,
        \"auto_init\": false
      }" || echo "WARNING: Failed to create repo, it may already exist"
...
- Synced charts, values, and applications"
...
- Tag: ${SERVICE_TAG}
- Synced: charts, values, applications" \
